...
<xsl:stylesheet version="1.0"
	...
	<xsl:template match="/">
		<xsl:variable name="grandTotal"
			select="sum(root/InvoiceReportDataList/InvoiceReportData/m_nAmount)" />
		<html>
			<body>
				<table class="trademust">
					<tr>
						<td class="print_formHeading" colspan="2">
							Invoice Report [From:<xsl:value-of select="root/m_strFromDate"/> - To:<xsl:value-of select="root/m_strToDate"/>]
          				</td>
					</tr>
					<tr>
									...
									<td colspan="2">
										<div style="min-height:1025px">
											<table class="print_tableBorder trademust">
												...
												<xsl:for-each select="root/InvoiceReportDataList/InvoiceReportData">
													<tr>
